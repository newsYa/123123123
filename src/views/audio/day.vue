<template>
  <div class="timeline-main">
    ...
    <div class="timeline-control">
      ...
      />
      <i
        class="menu-icon icon-right"
        :class="{'menu-icon-disabled': playing}"
        @click="forward"
      />
      <i
        class="menu-icon icon-up"
        :class="{'menu-icon-disabled': playing}"
        @click="speedSlow"
      />
      <i
        class="menu-icon speed"
      >{{ options.speed }}</i>
      <i
        class="menu-icon icon-down"
        :class="{'menu-icon-disabled': playing}"
        @click="speedQuick"
      />
    </div>
  ...
  .timeline-axis {
    ...
    .axis-item {
      ...
      .axis-item-tick {
        ...
        width: 10px;
        height: 10px;
        background: rgba(119, 24, 24, 0.5);
        transition: background 0.3s;
        cursor: pointer;
        &:hover {
          background: rgb(121, 39, 39);
        }
      }
